...
def get_node_argument_value(
    node: FieldNode | DirectiveNode, arg_name: str, variables: dict[str, Any]
) -> Any:
    ...
    if not arg:
        raise ValueError(
            f"Value for {arg_name!r} not found in {node.name.value!r} arguments"
        )
